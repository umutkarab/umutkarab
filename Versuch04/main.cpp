...
int main()
{
    ...
    Vektor Mensch(0, radiusErde + (hoehePlattform + hoeheMensch)/1000, 0); // we div with 1000 to turn it to km
    Vektor newSub(0, 0, 0);
    double newSkalar = 0;
    ...
    {
    	Erde.rotiereUmZ(increment); // world"s vector x grows bigger and y grows smaller, so it rotates to right side
    	zaehler++;
    	newSub = Mensch.sub(Erde); // it is basically the vector of what person sees
    	...
    	{
    		Senkrecht = true; // we stop the while loop
    		Sichtweite = newSub.laenge(); // how much we see is, our last newSub value
       	}
    }

        double Grad = Mensch.winkel(Erde) * 180 /PI; // winkel B
