...
int main()
{
    ...
    do  // Anzeige des Menüs
    {
        std::cout << std::endl
                  ...
                  << "(6): Alle ausgeliehenen ausgeben" << std::endl
                  << "(7): Beenden" << std::endl;
...
void alleAusgeliehenenAusgeben(std::vector<Medium*>& medien)
{
    ...
    bool ausgelVorhanden=false;
    for (unsigned int index = 0; index < medien.size(); index++)
    {
        if(medien[index]->getStatus()) {
            std::cout << "*************************************************************" << std::endl;
            std::cout << *medien[index];
            ausgelVorhanden = true;
        }
    }
    if(!ausgelVorhanden) std::cout << "Keine." << std::endl;
}
